...
public class TerminalItem
{
    ...
    public class InfoItem
    {
        ...
        double dis;
        int pre;
        public InfoItem(int origin,double dis,int pre)
        {
            this.origin=origin;
            this.dis=dis;
            this.pre=pre;
        ...
        public void change(int origin,double dis,int pre)
        {
            this.origin=origin;
            this.dis=dis;
            this.pre=pre;
        ...
        for(int i=vStart;i<vEnd;i++)
        {
            ...
            for(TreeHubSub.VInfoItem vInfoItem:thb.array[v])
            {
                ...
                for(int h=hStart;h<hEnd;h++)
                {
                    ...
                    {
                        dynamicHL[dynamicHop]=new InfoItem(vInfoItem.origin,hl.disOfLabel[h],hl.preOfLabel[h]);
                    }
                    else if(hl.disOfLabel[h]+1e-6<=dynamicHL[dynamicHop].dis)
                    {
                        dynamicHL[dynamicHop].change(vInfoItem.origin,hl.disOfLabel[h],hl.preOfLabel[h]);
                    }
                }
            }
